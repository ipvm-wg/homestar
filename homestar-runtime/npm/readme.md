...
## darwin arm64
cargo build -p homestar-runtime --locked --release --target aarch64-apple-darwin
...
## darwin x64
cross build -p homestar-runtime --locked --release --target x86_64-apple-darwin
export node_os=darwin
...
## linux arm64
cross build -p homestar-runtime --locked --release --target aarch64-unknown-linux-gnu
...
## linux x64
cross build -p homestar-runtime --locked --release --target x86_64-unknown-linux-musl
export node_os=linux
export node_arch=x64
export node_pkg="${bin}-${node_os}-${node_arch}"
mkdir -p "binaries/${node_pkg}/bin"
